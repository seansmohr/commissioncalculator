...
test('Pennsylvania Long Term Care uses the no-downline scale', function () {
  var p = 'Long Term Care - Individual (new business)';
  var g = 'Long Term Care - Association / Sponsored Group / Common Employer (new business)';
  var products = engine.getProducts('Mutual of Omaha', 'PA');
  // Pennsylvania now offers the same two products as CA and VA, at its own rates.
  assert.deepStrictEqual(products.sort(), [g, p].sort());
  close(engine.findRule('Mutual of Omaha', 'PA', p, 65).rate, 0.50, 'PA individual under 70');
  close(engine.findRule('Mutual of Omaha', 'PA', p, 72).rate, 0.30, 'PA individual 70-74');
  close(engine.findRule('Mutual of Omaha', 'PA', p, 77).rate, 0.25, 'PA individual 75-79');
  close(engine.findRule('Mutual of Omaha', 'PA', g, 65).rate, 0.45, 'PA group under 70');
  // California and Virginia are unaffected and still pay the higher scale.
  close(engine.findRule('Mutual of Omaha', 'CA', p, 65).rate, 0.60, 'CA is unchanged');
  close(engine.findRule('Mutual of Omaha', 'VA', p, 65).rate, 0.60, 'VA is unchanged');
});
...
test('GTL rules explain their source without asking for an impossible check', function () {
  DATA.rules.filter(function (r) { return r.carrier === 'GTL'; }).forEach(function (r) {
    assert.ok(!r.verify, r.product + ' should not be flagged - there is no fuller schedule to check');
    assert.ok(r.note && r.note.indexOf('no state or age breakdown') !== -1,
      r.product + ' should still say the source carries no state or age breakdown');
  });
});
